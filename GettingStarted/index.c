...
Third parties libraries could be (or become) adversarial. 
In 42 Libraries are forced to use only explicitly provided resources,
stopping library hakers.
The software architecture of a 42 program can enforce that important actions
are only performed when precise security conditions are met.
</li><li>
<h5>Performance by Abstraction</h5>
Caching and parallelism are the two main ways to optimize code.
Caching avoids doing the same operation over and over again,
and Parallelism allows doing more things at the same time.
Writing optimized code is hard, and many optimizations may break the code in unexpected
 and hard to debug ways.
In 42 supports correct by construction caching and parallelism.
This means that those optimizations are easy to insert and are unable
to impact the behaviour of the system
</li><li>
<h5>Customization by Composition</h5>
In 42 code can be used, reused, composed and customized in a very flexible way.
This allows to mandate dependencies and programming style from an architectural
designer standpoint.
...
on writing the important bits of their code, while all the boiler-plate code is automatically generated.
</li><li>

<h5>Dependendecy hell is over</h5>
In 42 has been conceived from the ground up so that 
a large number of third party libraries, and even multiple 
versions of the same library, can transparently coexists.
This all works without the need of any external tool like Maven or Pip.
</li><li>

<h5>Deployment made trivial</h5>
...
All integrated within the same coding environment.
</li><li>

<h5>No need of any operative system knowledge</h5>
...
know about shell commands, system conventions, environment variables, character encodings
and anything else usually connected with a specific operative system.
</li><li>

<h5>Bounded complexity</h5>
Very abstract language features like Lambdas and Generics allows some mathematically 
adept programmers to write very compact and reusable code,
while being one of the biggests roadblocks for everyone else.

In 42 lambdas and generics are replaced with other, more intuitive features.
</li><li>


<h5>A typed language with flexible syntax</h5>
42 is a typed language, like Java and C#. This allows to catch errors earlier and, most importantly,
allowing tools and libraries to support the developer since they can have
 a rich static undestanding of the code base.
42 have a flexible syntax, like python. This allows to elegantly express many different concepts.
</li><li>
...
It may be a poorly designed algorithm, abstraction mismatch, or maintenance spoiling your perfomance by slowly twisting assumptions.
In 42 various forms of caching and automatic parallelism are proven semantically unobservable.
This means that those optimizations are easy to insert and are unable
to impact the behaviour of the system
</li><li>
