m4_include(`header.h')m4_dnl
OFloatClass(index)
<div class="rotate90"> Index of Content</div>
ContinueFloat
<ol>
WMenuItem(`#Basics',`Basic concepts')
WMenuItem(`#ModifiersIntro',`Introduction to modifiers')
WMenuItem(`#BasicClasses',`Basic Classes')
WMenuItem(`#ErrorsAndExceptions',`Errors and Exceptions')
WMenuItem(`#Caching',`Caching')
WMenuItem(`#CachingMut',`Caching on Mutable objects')
WMenuItem(`#InterfacesAndData',`Interfaces and Data')
WMenuItem(`#Sequences',`Collections and Sequences')
WMenuItem(`#InputOutput',`Input Output')
WMenuItem(`#Exercises',`Exercises')
WMenuItem(`#MetaprogrammingIntro',`Metaprogramming Introduction')
WMenuItem(`#Refactoring',`Programmatic Refactoring')
WComm WMenuItem(`#MoreDecorators',`More Decorators')

WComm WMenuItem(`#ExamplesOfLibraries',`Examples of libraries')
WComm WMenuItem(`#MetaprogrammingIntro',`Introduction to Metaprogramming')
WComm WMenuItem(`#DeployCode',`Deploy code')
WComm WMenuItem(`#Philosophy',`Guaranties and Philosophy')
</ol>
CFloat

WChapter(`Basics',`tutorial1Basics.c')
WChapter(`ModifiersIntro',`tutorial2ModifiersIntro.c')
WChapter(`BasicClasses',`tutorial3BasicClasses.c')
WChapter(`ErrorsAndExceptions',`tutorial4ErrorsAndExceptions.c')
WChapter(`Caching',`tutorial5Caching.c')
WChapter(`CachingMut',`tutorial6CachingMut.c')
WChapter(`InterfacesAndData',`tutorial7InterfacesAndMatching.c')
WChapter(`Sequences',`tutorial8Sequences.c')
WChapter(`InputOutput',`tutorial9InputOutput.c')
WChapter(`Exercises',`tutorial10Exercises.c')
WChapter(`MetaprogrammingIntro',`tutorial11MetaprogrammingIntro.c')
WChapter(`Refactoring',`tutorial12Refactoring.c')
WComm WChapter(`MoreDecorators',`tutorial13MoreDecorators.c')
WComm WChapter(`ExamplesOfLibraries',`tutorial10ExamplesOfLibraries.c')
WComm WChapter(`DeployCode',`tutorial12DeployCode.c')
WComm WChapter(`Philosophy',`tutorial13Philosophy.c')
m4_include(`footer.h')
